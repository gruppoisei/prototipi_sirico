...
export class ModificaPasswordComponent {
  ...
  passwordMatchValidator(control: AbstractControl) {
    return control.get('nuovaPassword')?.value === control.get('confermaPassword')?.value ? null : {mismatch: true}
  }

  isFieldValid(field: string) {
    return this.cambioPasswordForm.get(field)?.touched && !this.cambioPasswordForm.get(field)?.valid
  }
}
