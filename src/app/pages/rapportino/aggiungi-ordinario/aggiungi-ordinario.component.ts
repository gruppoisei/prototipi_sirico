...
export class AggiungiOrdinarioComponent {
  ...
  AggiungiAttivitaGiorno() {
    if (
      ...
      this.rapportinoService.AggiungiAttivitaGiorno(this.attivitaDaAggiungere).subscribe((res) => {
        try {
          if (res > 0) {
            ...
            aggiornoComponenteAttivitaGiorno.oreLavorate = this.attivitaDaAggiungere.oreLavorate
            aggiornoComponenteAttivitaGiorno.oreStraordinario = this.attivitaDaAggiungere.oreStraordinario
            aggiornoComponenteAttivitaGiorno.sedeLavoro = this.rapportinoService.infoPersona.listaSedeLavoroPersona.find(
              (e) => e.sedeLavodoPersonaId == this.attivitaDaAggiungere.sedeLavoroPersonaId
            )?.nomeSedeLavoro
            aggiornoComponenteAttivitaGiorno.nomeProgetto = this.rapportinoService.infoPersona.listaAttivitaProgettoPersona.find(
              (e) => e.attivitaProgettoPersonaId == this.attivitaDaAggiungere.attivitaPersonaId
            )?.nomeProgetto
      ...
      alert('uno o più campi non validi')
    }
  }
}
