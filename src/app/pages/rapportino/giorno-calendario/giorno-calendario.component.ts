...
export class GiornoCalendarioComponent {
  ...
  ngOnInit(): void {
    if (this.rapportinoService.risposta.rapportino.dataRapportino != undefined) {
      ...
      this.giorno.dataNumero! < 10
        ? (this.giornoString = this.giornoString.slice(0, 8) + 0 + this.giorno.dataNumero)
        : (this.giornoString = this.giornoString.slice(0, 8) + this.giorno.dataNumero)
    ...
    for (let i = 0; i < this.giorno.listaAttivitaGiorno.length; i++) {
      ...
      if (prog == -1)
        this.rapportinoService.oreProgetto.push({
          ...
          oreProgetto: this.giorno.listaAttivitaGiorno[i].oreLavorate + this.giorno.listaAttivitaGiorno[i].oreStraordinario,
        })
      else
        this.rapportinoService.oreProgetto[i].oreProgetto +=
          this.giorno.listaAttivitaGiorno[i].oreLavorate + this.giorno.listaAttivitaGiorno[i].oreStraordinario
    }
  ...
  VerificaMeseEsatto(): boolean {
    let ver =
      this.rapportinoService.oggi.getMonth() == Number(this.rapportinoService.risposta.rapportino.dataRapportino?.toString().split('-')[1]) - 1
    return ver
  }
